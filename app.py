...
if current_user == "admin2":
    ...
    if not st.session_state.admin2_authenticated:
        st.subheader("🔒 Weryfikacja tożsamiści systemu ratunkowego")
...
with st.expander("🎨 Personalizacja Wyglądu i Zarządzanie Kontem"):
    ...
    new_nick = st.text_input("Zmień swój stały podpis (nick) (litery 'j' zostaną zmienione na 'i'):", value=user_saved_nick)
    processed_new_nick = new_nick.replace('j', 'i').replace('J', 'I')
    if processed_new_nick != user_saved_nick:
        st.session_state.global_store["user_data"][current_user]["saved_nick"] = processed_new_nick.strip()
        save_global_data(st.session_state.global_store)
        st.rerun()

    # --- NOWA SFINOMALIZOWANA SEKCJA: ZWIJANY CZAT POMOCNICZY NA SAMYM DOLE EXPANDERA ---
    # Dostęp tylko dla osób z rangą (Właściciel, Admin, Moderator)
    if is_staff:
        st.write("---")
        with st.expander("💬 Prywatny Czat Pomocniczy (Kliknij, aby rozwinąć)", expanded=False):
            st.write("### Wiadomości panelu pomocniczego")
            ...
            if not staff_messages:
                st.caption("Brak wiadomości na czacie pomocniczym.")
            else:
                for idx, msg in enumerate(staff_messages):
                    fallback_color = "#FF4B4B" if msg.get("sender_role") == "Admin" else "#FFA500"
                    current_bar_color = msg.get("bar_color", fallback_color)
                    is_my_own_message = (msg.get("author_key") == current_user)
                    
                    ch_col1, ch_col2 = st.columns([8, 2])
                    with ch_col1:
                        # Formatowanie bez dodatkowych nawiasów w randze zgodnie z wytycznymi
                        st.markdown(f"🟢 **{msg.get('sender_nick')}** {msg.get('sender_role')}: {msg.get('text')}")
                    
                    with ch_col2:
                        # Możliwość usuwania wyłącznie własnych wiadomości
                        if is_my_own_message:
                            if st.button("Usuń", key=f"del_exp_msg_{idx}"):
                                current_data = load_global_data()
                                if idx < len(current_data["staff_chat"]):
                                    current_data["staff_chat"].pop(idx)
                                    save_global_data(current_data)
                                    st.session_state.global_store = current_data
                                    st.rerun()

            # Formularz wysyłania wiadomości
            with st.form("expander_staff_chat_form", clear_on_submit=True):
                role_label = "Admin" if is_admin else "Moderator"
                staff_nick = user_saved_nick if user_saved_nick else f"User_{current_user[:6]}"
                
                chat_msg_input = st.text_input(f"Wiadomość jako {staff_nick}:", placeholder="Wpisz treść (litery 'j' zmienią się w 'i')...")
                processed_chat_msg = chat_msg_input.replace('j', 'i').replace('J', 'I')
                
                submit_exp_chat = st.form_submit_button("Wyślij na czat pomocniczy")
                if submit_exp_chat and processed_chat_msg.strip():
                    time_stamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M')
                    formatted_msg = {
                        "sender_nick": staff_nick, "sender_role": role_label, "time": time_stamp,
                        "text": processed_chat_msg.strip(), "bar_color": staff_bar_color, "author_key": current_user  
                    }
                    current_data = load_global_data()
                    if "staff_chat" not in current_data: current_data["staff_chat"] = []
                    current_data["staff_chat"].append(formatted_msg)
