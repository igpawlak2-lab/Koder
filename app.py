...
with st.form("comment_form", clear_on_submit=True):
    ...
    if wyslij and komentarz_tekst.strip():
        ...
        # --- WYWODŁANIE POWIADOMIENIA O NOWYM KOMENTARZU ---
        wyzwol_powiadomienie_systemowe(
            autor=podpis,
            tresc=komentarz_tekst.strip(),
            nazwa_czatu="Nowy Komentarz"
        )
            ...
            with cc2:
                if is_staff or is_real_admin:
                    ...
                    if st.button("❌ Usuń", key=f"del_com_{idx}", type="primary", use_container_width=True):
                        ...
                        st.session_state.global_store = current_data
                        st.rerun()
                        # --- INICJALIZACJA POWIADOMIEN PRZY WEJŚCIU NA STRONĘ ---
components.html("""
    <script>
