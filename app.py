...
# Domyślny układ klocków na stronie dla adminów
DEFAULT_LAYOUT = ["panel_sterowania", "tablica_ogloszen", "notatnik", "chat_pomocniczy", "historia"]
...
# Gwarancja obecności klucza układu
if "dashboard_layout" not in user_profile:
    user_profile["dashboard_layout"] = list(DEFAULT_LAYOUT)
    st.session_state.global_store["user_data"][current_user]["dashboard_layout"] = list(DEFAULT_LAYOUT)
    save_global_data(st.session_state.global_store)
...
current_layout = user_profile.get("dashboard_layout", list(DEFAULT_LAYOUT))

# Posiłkowe kontenery do dynamicznego układania elementów (C1 i C2)
containers = {
    "left": st.container(),
    "right": st.container()
}

# Deklarujemy wewnętrzne funkcje budujące konkretne klocki UI
def render_panel_sterowania():
                    ...
                    if chat_type == "👥 Grupa Ogólna Staffu":
                        ...
                        with st.container(height=450):
                            if not staff_messages:
                                p_cap = st.caption("Brak wiadomości.")
                            else:
...
# Słownik mapujący identyfikatory modułów na funkcje wykonawcze
RENDER_MAPPING = {
    "panel_sterowania": render_panel_sterowania,
    "tablica_ogloszen": render_tablica_ogloszen,
    "notatnik": render_notatnik,
    "chat_pomocniczy": render_chat_pomocniczy,
    "historia": render_historia
}

# --- DYNAMICZNY PODZIAŁ I RENDEROWANIE ZGODNIE Z UKŁADEM ---
# Renderujemy elementy zgodnie z zapisaną w profilu kolejnością
for module_id in current_layout:
    if module_id in RENDER_MAPPING:
        RENDER_MAPPING[module_id]()
...
with st.expander("🎨 Personalizacja Wyglądu i Zarządzanie Kontem"):
    ...
    # --- NOWA SEKCJA: PRZESTAWIANIE ELEMENTÓW DLA ADMINÓW/KADRY ---
    if is_staff:
        st.write("---")
        st.subheader("⚙️ Zarządzanie układem elementów strony głównej")
        st.write("Użyj strzałek, aby zmienić kolejność wyświetlania modułów na pulpicie. Twój układ jest zapisywany niezależnie.")
        
        # Słownik czytelnych nazw dla klocków UI
        friendly_names = {
            "panel_sterowania": "🎛️ Panel Sterowania / Chat Staffu",
            "tablica_ogloszen": "📢 Tablica Ogłoszeń",
            "notatnik": "📝 Twój Prywatny Notatnik",
            "chat_pomocniczy": "💬 Centrum Pomocy Technicznej (Support)",
            "historia": "📋 Historia operacji"
        }
        
        for i, layout_item in enumerate(current_layout):
            col_name, col_up, col_down = st.columns([4, 1, 1])
            with col_name:
                st.markdown(f"**{i+1}. {friendly_names.get(layout_item, layout_item)}**")
            with col_up:
                if i > 0:
                    if st.button("⬆️ Góra", key=f"lay_up_{layout_item}_{i}"):
                        current_layout[i], current_layout[i-1] = current_layout[i-1], current_layout[i]
                        st.session_state.global_store["user_data"][current_user]["dashboard_layout"] = current_layout
                        save_global_data(st.session_state.global_store)
                        st.rerun()
            with col_down:
                if i < len(current_layout) - 1:
                    if st.button("⬇️ Dół", key=f"lay_down_{layout_item}_{i}"):
                        current_layout[i], current_layout[i+1] = current_layout[i+1], current_layout[i]
                        st.session_state.global_store["user_data"][current_user]["dashboard_layout"] = current_layout
                        save_global_data(st.session_state.global_store)
                        st.rerun()
